"""
QuALITY Benchmark â€” Baseline vs Graph no markdown vs Graph with markdown.
Dataset: tasksource/QuALITY â€” Long-document QA (articles 2,000-6,000 words).
265 unique articles, 4,609 QA pairs total.

Ground truth: correct option text (from gold_label index).
Run: uv run python experiments/quality_benchmark.py
"""
import os
import sys
import random
...
# --- CONFIG ---
DATA_ROOT           = os.path.join(_REPO_ROOT, "data", "quality")
...
# Use 500 QA pairs sampled from all articles
SAMPLE_SIZE = 500
SEED        = 42
...
def load_quality() -> tuple:
    """Load QuALITY dataset. Returns (articles_dict, qa_list)."""
    from datasets import load_dataset
    print("[Data] Loading QuALITY (tasksource/QuALITY) from HuggingFace...")
    ds = load_dataset("tasksource/QuALITY")

    # Combine train + validation
    all_data = list(ds["train"]) + list(ds["validation"])
    print(f"[Data] Total {len(all_data)} QA pairs from {len(set(item['article_id'] for item in all_data))} articles.")

    # Build articles dict (article_id -> article text + title)
    articles = {}
    ...
    # Build QA list with ground truth = correct option text
    qa_pairs = []
    for item in all_data:
        gold_idx = item["gold_label"] - 1  # gold_label is 1-indexed
        options = item["options"]
        if 0 <= gold_idx < len(options):
            ground_truth = options[gold_idx]
        else:
            ground_truth = options[0]  # fallback
    ...
    # Collect only the articles referenced by sampled QA pairs
    used_article_ids = set(qa["article_id"] for qa in qa_pairs)
    used_articles = {aid: articles[aid] for aid in used_article_ids}

    print(f"[Data] Using {len(qa_pairs)} QA pairs from {len(used_articles)} articles (seed={SEED}).")
    return used_articles, qa_pairs


def prepare_files(articles: dict):
    """Write .md (Graph with markdown) and .txt (Baseline/Graph no markdown)."""
    print(f"\n[Prep] Writing context files for {len(articles)} articles...")

    for aid, info in tqdm(articles.items(), desc="Writing files"):
        title = info["title"]
        article = info["article"]

        # --- .md file (Graph with markdown) ---
        # Split article into paragraphs and assign markdown headings
        md_path = os.path.join(PARSED_DIR, f"{aid}.md")
        if not os.path.exists(md_path):
            paragraphs = [p.strip() for p in article.split("\n\n") if p.strip()]
            md = f"# {title}\n\n"
            for i, para in enumerate(paragraphs):
                # Use ## headings to create semantic sections
                md += f"## Section {i+1}\n{para}\n\n"
            with open(md_path, "w", encoding="utf-8") as f:
                f.write(md)

        # --- .txt file (Baseline / Graph no markdown) ---
        txt_path = os.path.join(BASE_PARSED_DIR, f"{aid}.txt")
...
def run_generation(ollama: OllamaManager, qa_list: list):
    ...
    for i, qa in enumerate(qa_list):
        ...
        for gen, store, top_k, label in [
            (baseline_gen,  baseline_results,  10,  "Baseline"),
...
def run_ragas(baseline_results, mdonly_results, graphnomd_results, graphmd_results):
    ...
    for label, results, out_csv in [
        ...
        try:
            ...
            print(f"=> Saved: quality/results/{out_csv}")
...
def print_comparison(scores: dict):
    ...
    print("\n" + "=" * 72)
    print(f"  QuALITY RESULTS ({SAMPLE_SIZE} QA pairs)")
    print("=" * 72)
...
def main():
    articles, qa_list = load_quality()
    prepare_files(articles)
